...
static void XMLCALL startElement(void *userData, const XML_Char *name, const XML_Char **attr)
{
    ...
    if(!strcasecmp(name, "gimbal")) {
        int valid=0;
        for (int i = 0; attr[i]; i += 2) {
            const char *an=attr[i];
            const char *av=attr[i+1];
            if(!strcasecmp(an, "ircmd")) {
                gpiovals->gpio=atoi(av);
                valid++;
            }else if(!strcasecmp(an, "irrepeat")) {
                gpiovals->delay=atoi(av);
                valid++;
            }else if(!strcasecmp(an, "gimbalcmd")) {
...
int udp_server_init(void) {
    return xTaskCreate((TaskFunction_t)udp_server, "GPIO udp server", DEFAULT_TASK_STACKSIZE, NULL, DEFAULT_TASK_PRIORITY, NULL);
}
